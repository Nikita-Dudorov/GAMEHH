...
void GameCore::loadSoundBuffers()
{
    ...
    if (!explosion_buffer.loadFromFile(Path_to_res + "sniper.wav"))
    {
    	printf("failure when loading sound buffer\n");
        exit(1);
    }
    SoundBuffers["explosion"] = explosion_buffer;
}
...
GameCore::runLoop(int x11, int x12, int y11, int y12, int x21, int x22, int y21, int y22, int scaleX, int scaleY,
    ...
    std::string Path_to_res = "C:/Box2D-master/game_includes/Resourses/";
...
void GameCore::createEntity(std::string object, int x, int y,
    ...
    if (object == "grenade") {
        class grenade grenade(world, window, Textures["grenade"], Textures["explosion"],SoundBuffers["explosion"],x,y);
        grenades.push_back(grenade);
    }
    if (object == "weapon") {
        class weapon weapon(world, window, Textures["ak47"], Textures["Bullet"],
...
void GameCore::loadTextures() {
    std::string Path_to_res = "C:/Box2D-master/game_includes/Resourses/";
    std::string Path_to_duck = "C:/Box2D-master/game_includes/Resourses/Duck Game Sprites/";
...
void GameCore::mapBuilder(std::string name){
    ...
    std::string s;
    outFile.open("C:/Box2D-master/game_includes/Resourses/Map/"+name+"Edited");
    inFile.open("C:/Box2D-master/game_includes/Resourses/Map/"+name);
